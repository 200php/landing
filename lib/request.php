<?php
	...
	if (isset($request["order"]) or isset($request["feedback"])) {
		...
		if (isset($request["order"])) {
			...
			// При заказе указаны наименования и количества веников
			if (isset($request["brooms"]) and $request["brooms"] and isset($request["q-new"]) and $request["q-new"]) {
				$order_list = null;
				$brooms_arr = $request["brooms"];				
				foreach ($request["q-new"] as $key => $value) {
					if ($value !== '') {
						$q_arr[] = $value;
					}
				}	
				if (count($brooms_arr) == count($q_arr)) {
					$order_arr = array_combine($brooms_arr, $q_arr);
					foreach ($order_arr as $key => $value) {
						$order_list .= "$key ($value шт.)\r\n";
					}				
					$data["message"] = "Сделал заказ:\r\n".$order_list;	
				}			
			}
		...
		if (isset($request["feedback"])) {
			...
			if (isset($request["brooms"]) and $request["brooms"] and isset($request["q-new"]) and $request["q-new"]) {
				...
				if (count($brooms_arr) == count($q_arr)) {
					...
					$order_list = "Сделал заказ:\r\n".$order_list;
				}
			}
			
			if (isset($order_list) and $order_list) $data["message"] = $order_list."\r\nСообщение:\r\n".$request["message"];
			else $data["message"] = $request["message"];
